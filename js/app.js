...
function displayPics(){
  ...
  // when changed to `const` we no longer can access `rando` because it is inside of the block and the call is outside.
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      // console.log(event.target.id + as ' +' h allProducts[i].votes + ' votes in ' + allProducts[i].views + ' views');
